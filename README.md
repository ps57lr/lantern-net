...
> **Development status:** the current source line is `0.3.0.dev6`. It combines the macOS browser-foreground correction with safe canonical serialization of multiline gateway-ping evidence. It is not a distributable dev6 family beta unless and until a clean Developer ID signed, Apple-notarized, stapled artifact passes every release gate. Even then, it remains a limited family beta—not production software, a remote-support service, an organization-wide network assessment, or an automatic repair tool. Use it only on computers and networks you own, manage, or are explicitly authorized to assess. The immutable `v0.3.0-dev4` release remains valid historical evidence, but its local page can open in Safari behind other windows and appear not to start. Dev5 was an untagged, non-distributed internal candidate: its foreground fix passed, but signed Finder low-impact smoke testing exposed the inherited multiline gateway-ping serialization failure, so no `v0.3.0-dev5` tag or handoff was created.
...
- A locked local-developer packaging pipeline for a visibly launched macOS app or Linux one-folder build. Dev4 established a clean, signed, Apple-notarized, stapled macOS family-beta artifact. The rejected internal dev5 candidate corrected browser foregrounding but failed low-impact smoke when multiline gateway-ping evidence reached presentation serialization. Dev6 combines both corrections and must independently pass the same offline verification, signing, notarization, stapling, launch, and clean-machine gates before handoff.
